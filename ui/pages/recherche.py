"""Liquisto — Unternehmens-Recherche (pipeline UI)."""
...
def _render_pdf_downloads(L: dict) -> None:
    ...
    with col_de:
        ...
        st.download_button(L["download_pdf_de"], data=pdf_de, file_name=file_name_de,
                           mime="application/pdf", use_container_width=True)
    with col_en:
        pdf_en, file_name_en = _build_pdf_export("en")
        st.download_button(L["download_pdf_en"], data=pdf_en, file_name=file_name_en,
                           mime="application/pdf", use_container_width=True)
...
def _render_briefing_tab(L: dict) -> None:
    ...
    if meeting_actions:
        action_icons = {"prepare_meeting": "🎯", "collect_missing_evidence": "🔍",
...
def _render_follow_up_panel(L: dict) -> None:
    ...
    if answer:
        dept_icons = {
            ...
            "BuyerDepartment": "🌐", "ContactDepartment": "👤", "SynthesisDepartment": "🧠",
        }
...
def _render_quality_tab(L: dict) -> None:
    ...
    if answer_matrix:
        ...
        status_colors = {"answered": "🟢", "partially_answered": "🟡", "blocked": "🔴", "pending": "⚪"}
        cols_per_row = 3
